...
const parse = id => {
  const root = api.getById(id);
  const children = getByParentId(id);
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  let append = document.getElementById("id");
...
function chart() {
    ...
    function update(source) {
        ...
        const duration = d3.event && d3.event.altKey ? 50 : 600;
        // Returns all nodes from "last" to root with reverse
        const nodes = root.descendants().reverse();
        // Returns linnks between all the nodes using source (parent) to target (child)
        ...
        .append("circle")
        .attr("r", 4)
        .attr("fill", d => (d._children ? "blue" : "#2a9fa3"))
